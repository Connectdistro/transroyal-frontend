...
Every material in this world is either **Physical** or **Digital**. There is no third category and no material should be ambiguous between the two.

- **Physical** — exists as a real object with mass: paint, rubber, concrete, glass, metal, skin, fabric. Lit by the scene's key/fill lights and environment map like everything else. Its own base color should read as a believable industrial hue under warm light — not blue, not fighting the lighting doctrine.
- **Digital** — represents information, not matter: a route line, a scan beam, a tracking indicator, a UI element rendered in-world. Typically `MeshBasicMaterial` already (unlit by design, not by accident) and stays in the brand's blue family regardless of the lighting doctrine, because it was never claiming to be a physical light source in the first place.

The test for any material: **if you removed this object, would the physical scene still make sense?** A truck's paint — yes, still a truck, just a different color. A route line — no, it's not a physical thing at all, it's the network's own visualization of a path. That's the line between the two categories, not "is it currently blue."
...
| **Open question — vehicle lighting** | Taillights use `ELECTRIC_400` (blue) rather than a realistic red/amber; headlights use `OFFWHITE_100` (already realistic) | N/A — stylistic choice, not a lighting-response question | **Needs your call**: is a blue taillight an intentional brand signature on physical equipment (keep), or should it move to realistic red/amber now that "blue = digital only" is the stated rule? Currently the only material that's genuinely ambiguous between the two categories. |

## What this doesn't decide

This document classifies. It doesn't recolor. The next initiative — recoloring the "Physical — body paint" and "Ground/surface" rows toward believable industrial tones (white/gray/graphite/muted per your own note against a monochromatic risk) — is a separate, larger pass across every chapter file's vehicle/structure materials, and should get its own scoped implementation plan before any code changes, matching how the lighting migration itself was handled.

One open decision before that pass can be scoped: the vehicle taillight question above. Worth resolving first, since it sets the precedent for how strictly "blue = digital only" is enforced everywhere else.
