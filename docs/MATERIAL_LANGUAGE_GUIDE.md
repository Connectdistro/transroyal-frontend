...
| **Brand** | Livery stripe on every vehicle `createTruck()`/`createVehicle()` builds except the yard service vehicle (Ground's highway fleet + dock/queued trucks, Pickup's courier van) — a thin `ELECTRIC_500` band along the body, added purely to say "this is TransRoyal," not to simulate a physical light | Yes, deliberately | Unlike every other Digital/Brand material in this codebase, the livery stripe is **lit** (`MeshStandardMaterial`, not `MeshBasicMaterial`) — real vinyl/paint striping is a physical surface and picking up the scene's warm key light is what keeps it reading as part of the vehicle's own paint job rather than a flat decal glued on top. This is the pattern going forward: reuse the project's existing canonical blue (`ELECTRIC_500`/`0x2f8bff`, same value as every Digital material and `tokens.css`'s `--accent`) rather than a new hex, but let a Brand material respond to lighting when the object it represents is genuinely physical (a stripe) rather than purely informational (a route line) — that's what "free to blend warm and blue" already meant. Don't force an existing Physical or Digital material into this category just because it's visually striking. |
